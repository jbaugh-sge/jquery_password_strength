  ...
  A.extend A.fn,
    ...
    checkPassword: (C, B) ->
      ...
      if C.length < B.minchar
        ...
        if C.length >= B.minchar and C.length <= (B.minchar + 2)
          F = (F + 6)
        else
          if C.length >= (B.minchar + 3) and C.length <= (B.minchar + 4)
            F = (F + 12)
          else
            F = (F + 18)  if C.length >= (B.minchar + 5)
      F = (F + 1)  if C.match(/[a-z]/)
      F = (F + 5)  if C.match(/[A-Z]/)
      F = (F + 5)  if C.match(/\d+/)
      F = (F + 7)  if C.match(/(.*[0-9].*[0-9].*[0-9])/)
      F = (F + 5)  if C.match(/.[!,@,#,$,%,^,&,*,?,_,~]/)
      F = (F + 7)  if C.match(/(.*[!,@,#,$,%,^,&,*,?,_,~].*[!,@,#,$,%,^,&,*,?,_,~])/)
      F = (F + 2)  if C.match(/([a-z].*[A-Z])|([A-Z].*[a-z])/)
      F = (F + 3)  if C.match(/([a-zA-Z])/) and C.match(/([0-9])/)
      F = (F + 3)  if C.match(/([a-zA-Z0-9].*[!,@,#,$,%,^,&,*,?,_,~])|([!,@,#,$,%,^,&,*,?,_,~].*[a-zA-Z0-9])/)
      D = 0

      while D < B.common.length
        F = -200  if C.toLowerCase() is B.common[D]
        D++
      F
) jQuery